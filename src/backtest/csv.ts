...
export function writeOptimizationCsv(metrics: ConfigMetrics[], path: string): string {
  ...
  const rows = metrics.map((m) => [
    ...
    m.candidate.skipCooldownWhenFlat === undefined ? "" : m.candidate.skipCooldownWhenFlat ? 1 : 0,
    m.candidate.confirmObservations ?? "",
    m.candidate.volPostpone === undefined ? "" : m.candidate.volPostpone ? 1 : 0,
    m.candidate.hardDrawdownPct ?? "",
